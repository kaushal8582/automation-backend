...
async function runCommand(bin, args, timeoutMs = 180_000) {
    return new Promise((resolve, reject) => {
        ...
        child.on('error', (error) => {
            clearTimeout(timer);
            reject(new AppError(`Failed to run ${bin}. Is FFmpeg installed? ${error.message}`, 500, 'IMPORT_PLATFORM_ERROR'));
        });
        child.on('close', (code) => {
            clearTimeout(timer);
            if (code === 0) {
            ...
            reject(new AppError(`${bin} failed (exit ${code}): ${stderr.slice(-500) || 'unknown error'}`, 500, 'IMPORT_PLATFORM_ERROR'));
        });
    });
}
export async function createImportTempDir() {
...
export async function writeStreamToFile(stream, filePath, maxBytes = env.MEDIA_MAX_UPLOAD_BYTES) {
    let written = 0;
    stream.on('data', (chunk) => {
        written += chunk.length;
        if (written > maxBytes) {
            stream.destroy(new AppError(`File exceeds maximum upload size of ${maxBytes} bytes`, 400, 'IMPORT_MEDIA_TOO_LARGE', { maxBytes }));
        }
    });
...
export async function muxVideoWithAudio(params) {
    ...
    if (!hasAudio) {
        throw new AppError('Failed to attach audio to video. The muxed file still has no sound.', 500, 'IMPORT_PLATFORM_ERROR');
    }
    logger.info('Muxed video with audio', { bytes: info.size, hasAudio });
    return { outputPath: params.outputPath, fileSize: info.size };
}
//# sourceMappingURL=media-mux.js.map