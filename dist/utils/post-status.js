 ...
 */
export function aggregatePostStatus(destinationStatuses) {
    ...
    if (published > 0 && published + failed + cancelled === total)
        return 'partially_published';
    if (processing > 0)
        return 'processing';
    // All waiting: pending = scheduled, queued = queued (immediate)
    const allWaiting = pending + queued + cancelled === total;
    if (allWaiting && pending > 0)
        return 'scheduled';
    if (allWaiting && queued > 0)
        return 'queued';
    return 'processing';
}
export function computeDestinationCounts(destinationStatuses) {
    return {
