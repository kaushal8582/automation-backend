...
import { CONTENT_IMPORT_DOWNLOAD_TIMEOUT_MS, CONTENT_IMPORT_MAX_REDIRECTS, } from '../constants/content-import.js';
 ...
 */
export async function safeFetchMediaStream(rawUrl, options) {
    ...
    for (let redirect = 0; redirect <= CONTENT_IMPORT_MAX_REDIRECTS; redirect += 1) {
        try {
            ...
            if (axios.isAxiosError(error) && error.response?.status) {
                throw new AppError(`Failed to download media (HTTP ${error.response.status})`, 502, 'IMPORT_DOWNLOAD_FAILED', { status: error.response.status });
            }
            throw new AppError(error instanceof Error ? error.message : 'Failed to download media', 502, 'IMPORT_DOWNLOAD_FAILED');
        }
    ...
    if (contentLength !== undefined && Number.isFinite(contentLength) && contentLength > maxBytes) {
        response.data.destroy();
        throw new AppError(`File exceeds maximum upload size of ${maxBytes} bytes`, 400, 'IMPORT_MEDIA_TOO_LARGE', { maxBytes, contentLength });
    }
    const contentTypeHeader = response.headers['content-type'];
    const contentType = typeof contentTypeHeader === 'string' ? contentTypeHeader.split(';')[0]?.trim() : undefined;
    ...
    upstream.on('data', (chunk) => {
        ...
        if (downloaded > maxBytes) {
            upstream.destroy();
            guarded.destroy(new AppError(`File exceeds maximum upload size of ${maxBytes} bytes`, 400, 'IMPORT_MEDIA_TOO_LARGE', { maxBytes }));
            return;
        }
        guarded.push(chunk);
    });
