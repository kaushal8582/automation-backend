 ...
 */
export function isLadderQuality(quality) {
...
export function rankVideoResource(resource) {
    ...
    if (/original/i.test(q))
        return 3_000_000 + sizeBoost;
    if (!isLadderQuality(q))
        return 2_000_000 + sizeBoost;
    const m = q.match(/(\d+)\s*p/i);
    const height = m ? Number(m[1]) : 0;
    return height * 1000 + sizeBoost;
}
export function pickPreferredVideo(videos) {
    if (videos.length === 0)
        return undefined;
    return [...videos].sort((a, b) => rankVideoResource(b) - rankVideoResource(a))[0];
}
//# sourceMappingURL=ig-video-rank.js.map