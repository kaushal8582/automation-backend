...
import { assertInstagramMediaUrl, permalinkContainsShortcode, } from './instagram-permalink.js';
 ...
 */
export async function resolveOwnedInstagramPermalink(userId, rawUrl, options) {
    ...
    if (accounts.length === 0) {
        throw new AppError('Connect the Instagram account that owns this reel, then paste the link again. Third-party public reels cannot be imported via Instagram’s official API.', 400, 'IMPORT_SOURCE_UNSUPPORTED');
    }
    ...
    for (const account of accounts) {
        let accessToken;
        try {
            accessToken = await metaTokenService.getDecryptedAccessToken(userId, account._id.toString());
        }
        catch (error) {
        ...
        for (let page = 0; page < MAX_PAGES_PER_ACCOUNT; page += 1) {
            ...
            try {
                ...
                if (error instanceof AppError) {
                    ...
                    if (error.code === 'IMPORT_TOKEN_EXPIRED' ||
            ...
            const match = listed.items.find((item) => permalinkContainsShortcode(item.permalink, parsed.shortcode));
            if (match) {
    ...
    throw new AppError(`Reel/post “${parsed.shortcode}” was not found on your connected Instagram account(s). Only media you own can be imported from a link — Instagram’s official API does not allow downloading other people’s public reels.`, 404, 'IMPORT_MEDIA_NOT_FOUND', { shortcode: parsed.shortcode });
}
//# sourceMappingURL=instagram-permalink-resolver.js.map