...
export class FacebookPublisher {
    ...
    async createMediaContainer(input) {
        try {
            ...
            const { data } = await this.client.post(`/${input.platformAccountId}/videos`, null, {
                params: {
    ...
    async checkMediaStatus(input) {
        try {
            ...
            const statusCode = videoStatus === 'ready'
                ? 'FINISHED'
                : videoStatus === 'error'
                    ? 'ERROR'
                    : 'IN_PROGRESS';
    ...
    async waitUntilVideoReady(input) {
        for (let attempt = 1; attempt <= this.maxPollAttempts; attempt += 1) {
            ...
            if (status.statusCode === 'ERROR') {
                throw new AppError(`Facebook video processing failed: ${status.status ?? 'error'}`, 502, 'META_MEDIA_PROCESSING_FAILED', { status: status.status, statusCode: status.statusCode });
            }
        }
        throw new AppError('Facebook video processing timed out', 504, 'META_MEDIA_PROCESSING_FAILED', { maxPollAttempts: this.maxPollAttempts, pollIntervalMs: this.pollIntervalMs });
    }
    /**
