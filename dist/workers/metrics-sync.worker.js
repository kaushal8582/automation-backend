...
import { METRICS_SYNC_QUEUE, } from '../queues/metrics-sync.queue.js';
const logger = createLogger('metrics-sync-worker');
export async function metricsSyncJob(job) {
    const data = job.data;
    if (data.type === 'post') {
...
export function createMetricsSyncWorker() {
    const worker = new Worker(METRICS_SYNC_QUEUE, metricsSyncJob, {
...
//# sourceMappingURL=metrics-sync.worker.js.map