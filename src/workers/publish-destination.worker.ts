...
import {
  PUBLISH_DESTINATION_QUEUE,
  type PublishDestinationJobData,
} from '../queues/publish-destination.queue.js';
import { Types } from 'mongoose';

const logger = createLogger('publish-destination-worker');

async function syncPostFromDestinations(postId: Types.ObjectId): Promise<void> {
...
export async function publishDestinationJob(
  job: Job<PublishDestinationJobData>,
): Promise<void> {
  ...
  if (!videoUrl.startsWith('http')) {
    throw new AppError(
      'Video public URL is unavailable. Set R2_PUBLIC_URL.',
      400,
  ...
  let result: { platformContainerId: string; platformPostId: string };
...
export function createPublishDestinationWorker(concurrency: number): Worker<PublishDestinationJobData> {
  const worker = new Worker<PublishDestinationJobData>(
    PUBLISH_DESTINATION_QUEUE,
    publishDestinationJob,
    {
