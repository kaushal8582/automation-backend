...
import {
  ...
  getR2Bucket,
  getR2Client,
  isR2ConfiguredForRealUploads,
  resolveAccessibleMediaUrl,
} from '../config/r2.js';
...
} from '../models/content-import-job.model.js';
import { MediaAsset, type IMediaAssetDocument } from '../models/media-asset.model.js';
...
import {
  assertValidInstagramPublicUrl,
  instagramPublicUrlImportProvider,
} from '../providers/content-import/instagram-public-url.provider.js';
import { resolveOwnedInstagramPermalink } from '../providers/content-import/instagram-permalink-resolver.js';
import type { PublicImportResource } from '../providers/content-import/public-url.types.js';
...
function toPublicJob(job: IContentImportJobDocument): PublicImportJob {
  return {
    id: job._id.toString(),
...
async function toPublicMedia(media: IMediaAssetDocument): Promise<PublicMediaAsset & {
  sourceType?: string;
...
function normalizeMimeType(raw?: string, fallback?: AllowedMediaMimeType): AllowedMediaMimeType {
  const cleaned = (raw ?? '').split(';')[0]?.trim().toLowerCase();
  if (cleaned && (ALLOWED_MEDIA_MIME_TYPES as readonly string[]).includes(cleaned)) {
    return cleaned as AllowedMediaMimeType;
  }
...
async function findExistingImport(
  ...
  sourcePlatform: SocialPlatform | 'url',
  sourceExternalId: string,
): Promise<IMediaAssetDocument | null> {
...
async function findExistingResourceImport(
  userId: string,
  sourcePlatform: SocialPlatform | 'url',
  sourceExternalId: string,
  sourceResourceId: string,
...
function guessMimeFromResource(resource: PublicImportResource): AllowedMediaMimeType | undefined {
  ...
  if (resource.type === 'video' || format === 'MP4') return 'video/mp4';
...
function sanitizeImportFilename(title: string, resource: PublicImportResource): string {
  const base = title
    .replace(/[^\w\s.-]/g, '')
    .trim()
    .slice(0, 80)
...
function downloadHeadersForUrl(url: string): Record<string, string> {
...
function pickCompanionAudio(
  resources: PublicImportResource[],
  videoResourceId: string,
): PublicImportResource | undefined {
...
function alternateVideosWithLikelyAudio(
  resources: PublicImportResource[],
  currentId: string,
): PublicImportResource[] {
...
async function downloadIgResourceToFile(
  resource: PublicImportResource,
  filePath: string,
): Promise<number> {
  let downloadUrl = resource.downloadUrl;
  if (
    (!downloadUrl || downloadUrl.startsWith('pending:')) &&
    ...
    if (!resolved) {
      throw new AppError(
        'Could not resolve Instagram media download URL (audio/video task failed).',
        502,
...
export async function previewInstagramPublicLink(_userId: string, url: string) {
  ...
  return {
    ...
    resources: parsed.resources.map((r) => ({
      ...
      importable:
        (r.type === 'video' || r.type === 'image' || r.type === 'audio') &&
        (!r.downloadUrl.startsWith('pending:') || Boolean(r.resourceContent)),
    })),
  };
}

export async function enqueueInstagramPublicImport(
  userId: string,
  input: {
    sourceUrl: string;
    resourceIds: string[];
  ...
  if (input.resourceIds.length > INSTAGRAM_PUBLIC_IMPORT_MAX_RESOURCES) {
    throw new AppError(
      `You can import at most ${INSTAGRAM_PUBLIC_IMPORT_MAX_RESOURCES} resources at a time`,
      400,
      'VALIDATION_ERROR',
    );
  }

  const { parsed, selected } = await instagramPublicUrlImportProvider.resolveResources(
    input.sourceUrl,
    input.resourceIds,
  );

  const jobs: PublicImportJob[] = [];

  for (const resource of selected) {
    if (!input.forceDuplicate) {
      const existing = await findExistingResourceImport(
        userId,
        'instagram',
        parsed.externalId,
        resource.id,
    ...
    await getContentImportQueue().add(
      'import-instagram-public',
      {
        ...
        externalId: parsed.externalId,
        sourceResourceId: resource.id,
        sourceType: 'instagram_public' as const,
...
async function streamUploadToR2(params: {
  ...
  mediaType: 'video' | 'image' | 'thumbnail' | 'audio';
  stream: NodeJS.ReadableStream;
  contentLength?: number;
  /** When set, uses users/{userId}/imports/{platform}/… keys */
  ...
  try {
    ...
    throw new AppError(
      error instanceof Error ? error.message : 'Failed to upload imported media to storage',
      502,
...
async function maybeImportThumbnail(params: {
  userId: string;
  thumbnailUrl?: string;
}): Promise<{ thumbnailR2Key?: string; thumbnailPublicUrl?: string; originalThumbnailUrl?: string }> {
  if (!params.thumbnailUrl) return {};
  try {
...
export async function listAccountMedia(
  ...
  const accessToken = await metaTokenService.getDecryptedAccessToken(userId, socialAccountId).catch(
    (error) => {
  ...
  const limit = Math.min(
    Math.max(options.limit ?? CONTENT_IMPORT_DEFAULT_PAGE_SIZE, 1),
    CONTENT_IMPORT_DEFAULT_PAGE_SIZE,
  );
  ...
  const existingMap = new Map(
    existing.map((m) => [m.sourceExternalId!, m._id.toString()]),
  );

  const items: ExternalMediaItem[] = result.items.map((item) => {
...
export async function enqueueAccountImport(
  userId: string,
  socialAccountId: string,
  externalIds: string[],
) {
  ...
  if (externalIds.length > CONTENT_IMPORT_MAX_BATCH) {
    throw new AppError(
      `You can import at most ${CONTENT_IMPORT_MAX_BATCH} items at a time`,
      400,
      'VALIDATION_ERROR',
    );
  }
...
export async function previewUrlImport(
  ...
  if (isInstagramMediaUrl(url)) {
    ...
    return {
      ...
      preview: {
        ...
        mimeType:
          matched.item.mediaType === 'image' || matched.item.mediaType === 'carousel'
            ? 'image/jpeg'
            : 'video/mp4',
  ...
  if (!adapter) {
    throw new AppError(
      'This source cannot be imported automatically. Paste an Instagram reel/post link from a connected account, a direct media file URL, or upload the file.',
      400,
...
export async function getImportJob(userId: string, jobId: string): Promise<PublicImportJob> {
...
export async function getImportJobs(userId: string, jobIds: string[]): Promise<PublicImportJob[]> {
...
export async function retryImportJob(userId: string, jobId: string): Promise<PublicImportJob> {
  if (!Types.ObjectId.isValid(jobId)) {
    throw new AppError('Invalid import job id', 400, 'VALIDATION_ERROR');
  }
...
async function updateJob(
  jobId: string,
  patch: Partial<IContentImportJobDocument>,
): Promise<IContentImportJobDocument | null> {
  return ContentImportJob.findByIdAndUpdate(jobId, { $set: patch }, { new: true });
}

async function processInstagramPublicImportJob(
  data: {
    jobId: string;
    userId: string;
    sourceUrl?: string;
  ...
  _job: IContentImportJobDocument,
): Promise<void> {
    ...
    if (resource.type === 'video') {
      ...
      if (!hasAudio) {
        ...
        if (companion) {
          ...
          throw new AppError(
            'This Instagram reel only returned a silent video from the downloader (no audio track and no separate audio file). Try another reel, or pick “Original” quality if available.',
            422,
      ...
      if (fetched.contentLength !== undefined) assertUploadSize(fetched.contentLength);
...
export async function processImportJob(data: {
  ...
  try {
    ...
    if (data.sourceType === 'url') {
      if (!data.sourceUrl) throw new AppError('Missing source URL', 400, 'IMPORT_SOURCE_UNSUPPORTED');
    ...
    const mimeType = normalizeMimeType(
      resolved.mimeType,
      details.mediaType === 'image' ? 'image/jpeg' : 'video/mp4',
    );
    assertAllowedMimeType(mimeType);
