...
import { Post, type IPostDocument } from '../models/post.model.js';
import {
  PostDestination,
  type IPostDestinationDocument,
} from '../models/post-destination.model.js';
...
export function toPublicDestination(dest: IPostDestinationDocument): PublicPostDestination {
  return {
...
export function toPublicPost(post: IPostDocument): PublicPost {
  return {
...
export async function syncPostFromDestinations(postId: Types.ObjectId): Promise<IPostDocument> {
...
export async function createPost(
  ...
  let thumbnailMediaId: Types.ObjectId | undefined;
  ...
  if (accounts.length !== uniqueAccountIds.length) {
    throw new AppError(
      'One or more social accounts not found',
      404,
  ...
  if (inactiveAccount) {
    throw new AppError(
      `Account ${inactiveAccount.username ?? inactiveAccount.id} is not active. Reconnect and try again.`,
      400,
  ...
  const initialDestStatus = isScheduled ? ('pending' as const) : ('queued' as const);
  ...
  const destinations = await PostDestination.insertMany(
  ...
  await Promise.all(
    destinations.map((dest) =>
      queue.add(
        'publish',
        { destinationId: dest._id.toString(), userId },
        {
...
export async function createPostsBatch(
  ...
  if (videos.length !== uniqueMediaIds.length) {
    throw new AppError(
      'One or more ready videos were not found',
      404,
      'MEDIA_NOT_FOUND',
    );
  }
...
): Promise<{ posts: PublicPost[]; total: number; limit: number; offset: number }> {
...
export async function getPostById(
  userId: string,
  postId: string,
): Promise<{ post: PublicPost; destinations: PublicPostDestination[] }> {
...
const CANCELLABLE_STATUSES: ReadonlySet<string> = new Set(['pending', 'queued']);
...
export async function cancelPost(
  ...
  if (cancellable.length === 0) {
    throw new AppError(
      'No destinations can be cancelled — all are already published or failed',
      400,
...
export async function retryPost(
...
): Promise<{ post: PublicPost; destinations: PublicPostDestination[]; retriedCount: number }> {
  if (!Types.ObjectId.isValid(postId)) {
    throw new AppError('Invalid post id', 400, 'INVALID_POST_ID');
  }
  ...
  if (toRetry.length === 0) {
    throw new AppError(
      'No failed destinations to retry',
      400,
      'NO_DESTINATIONS_TO_RETRY',
    );
  }
  ...
  await Promise.all(
    toRetry.map(async (dest) => {
      ...
      await PostDestination.updateOne(
        { _id: dest._id },
        {
          $set: {
            status: 'queued',
