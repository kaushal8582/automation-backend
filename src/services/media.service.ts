...
async function toPublic(media: IMediaAssetDocument): Promise<PublicMediaAsset> {
  return {
...
function assertOwnedKey(userId: string, r2Key: string): void {
...
export async function createPresignedUpload(
  ...
  const mediaType = resolveMediaType(input.mimeType as AllowedMediaMimeType, input.type);
  const r2Key = buildObjectKey(userId, input.mimeType as AllowedMediaMimeType, mediaType);
...
export async function createPresignedUploadBatch(
  userId: string,
  input: PresignMediaBatchInput,
): Promise<{ uploads: Array<PresignedUploadResult & { clientIndex: number }> }> {
  const uploads = await Promise.all(
    input.files.map(async (file, clientIndex) => {
...
export async function completeMediaUpload(
  ...
  try {
    ...
    throw new AppError(
      'Uploaded object not found in R2. Complete the upload before confirming.',
      400,
      'MEDIA_NOT_FOUND_IN_STORAGE',
    );
  }
  ...
  if (remoteSize !== input.fileSize) {
    throw new AppError(
      'Uploaded file size does not match the declared fileSize',
      400,
      'MEDIA_SIZE_MISMATCH',
      { expected: input.fileSize, actual: remoteSize },
    );
  }
...
export async function listMediaForUser(userId: string): Promise<PublicMediaAsset[]> {
...
export async function deleteMediaForUser(userId: string, mediaId: string): Promise<void> {
  if (!Types.ObjectId.isValid(mediaId)) {
    throw new AppError('Invalid media id', 400, 'INVALID_MEDIA_ID');
  }
  ...
  const related: IMediaAssetDocument[] = [];
  if (
    media.type === 'video' &&
...
export async function bulkDeleteMediaForUser(
  userId: string,
  ids: string[],
  ...
  const relatedAudioIds = new Set<string>();
  for (const media of items) {
    if (
      ...
      for (const s of siblings) relatedAudioIds.add(s._id.toString());
    }
  }
...
async function deleteR2Keys(keys: string[]): Promise<void> {
  if (!isR2ConfiguredForRealUploads()) return;
...
async function deleteMediaAssetsAndR2(
  userId: string,
  items: IMediaAssetDocument[],
): Promise<void> {
  const r2Keys: string[] = [];
  for (const media of items) {
    if (media.r2Key) r2Keys.push(media.r2Key);
    if (media.thumbnailR2Key) r2Keys.push(media.thumbnailR2Key);
  }
