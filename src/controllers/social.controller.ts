...
export async function listInstagramAccountsHandler(
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> {
...
export async function connectManualAccount(
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> {
  try {
    const account = await connectManualSocialAccount(
      req.user!.id,
      req.body as ManualSocialAccountInput,
    );
...
export async function removeAccount(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
...
export async function testPublishInstagram(
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> {
  try {
    const result = await testPublishInstagramReel(
      req.user!.id,
      req.body as InstagramTestPublishInput,
    );
...
export async function instagramConnect(
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> {
...
export async function instagramExchange(
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> {
...
export async function facebookCallback(
  ...
  try {
    ...
    const frontend = process.env.FRONTEND_URL?.replace(/\/$/, '') ?? 'http://localhost:3000';
    const message =
      error instanceof Error ? error.message : 'Facebook OAuth callback failed';
    res.redirect(302, `${frontend}/accounts?error=${encodeURIComponent(message)}`);
  }
}

export async function listFacebookAccountsHandler(
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> {
...
export async function reconnectAccount(
  ...
  try {
    ...
    if (account.platform !== 'instagram') {
      throw new AppError(
        'Reconnect via OAuth is only implemented for Instagram in Phase 6',
        501,
        'RECONNECT_NOT_IMPLEMENTED',
      );
    }
