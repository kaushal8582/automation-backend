...
function toExternalItem(node: FbVideoNode, accountId: string): ExternalMediaItem {
  return {
    externalId: node.id,
    platform: 'facebook',
    accountId,
 ...
 */
export class FacebookImportProvider implements ContentImportProvider {
  ...
  async listMedia(input: ListMediaInput): Promise<ListMediaResult> {
    ...
    try {
      const { data } = await this.client.get<FbVideoListResponse>(
  ...
  async getMediaDetails(input: GetMediaDetailsInput): Promise<ExternalMediaItem> {
    try {
      const { data } = await this.client.get<FbVideoNode>(`/${input.externalId}`, {
        params: {
          fields: 'id,description,source,picture,created_time,length,permalink_url',
          access_token: input.accessToken,
        },
  ...
  async resolveMediaSource(input: ResolveMediaSourceInput): Promise<ResolvedMediaSource> {
    ...
    if (!item.sourceUrl) {
      throw new AppError(
        'Facebook did not return a downloadable video source URL. Reconnect the page or check permissions.',
        404,
