...
import type {
  ...
  PublicImportResourceType,
  PublicUrlImportProvider,
  PublicUrlParseResult,
} from './public-url.types.js';
...
type ProviderParsePayload = {
  status?: boolean | string | number;
  status_code?: number;
  message?: string;
  data?: {
    media?: ProviderMediaNode[];
    resources?: unknown[];
    title?: string;
    caption?: string;
    thumbnail?: string;
...
function getIgAuth(): string {
  const auth = env.IG_AUTH ?? env.VIDSSAVE_AUTH;
  if (!auth) {
    throw new AppError(
      'Instagram public link import is not configured. Set IG_AUTH on the server.',
      503,
...
function mapProviderError(error: unknown): never {
  ...
  if (axios.isAxiosError(error)) {
    ...
    if (status && status >= 500) {
      throw new AppError(
        'Instagram download provider is temporarily unavailable.',
        502,
        'INSTAGRAM_PROVIDER_UNAVAILABLE',
      );
    }
  ...
  throw new AppError(
    error instanceof Error ? error.message : 'Failed to parse Instagram media',
    502,
...
function hasParsePayload(payload: ProviderParsePayload): boolean {
  ...
  if (typeof payload.status_code === 'number' && payload.status_code >= 400) return false;
...
function pickDownloadUrl(raw: Record<string, unknown>): string {
  const candidates = [
    raw.download_url,
    raw.downloadUrl,
    raw.preview_url,
...
function normalizeResourceType(rawType: unknown, format: string, quality = ''): PublicImportResourceType {
...
function stableResourceId(downloadUrl: string, index: number, quality: string, type: string): string {
  const hash = createHash('sha1').update(`${downloadUrl}|${quality}|${type}|${index}`).digest('hex').slice(0, 16);
  return `ig-${hash}`;
}

function normalizeOneResource(raw: unknown, index: number, labelHint?: string): PublicImportResource | null {
  const rec = asRecord(raw);
  if (!rec) return null;
  let downloadUrl = pickDownloadUrl(rec);
  const resourceContent =
    typeof rec.resource_content === 'string' ? rec.resource_content.trim() : '';

  // Audio often has empty download_url and only resource_content — keep it for later resolve.
  if (!downloadUrl && !resourceContent) return null;
  ...
  const qualityRaw =
    (typeof rec.quality === 'string' && rec.quality.trim()) ||
  ...
  return {
    ...
    downloadUrl,
    downloadMode:
      (typeof rec.downloadMode === 'string' && rec.downloadMode) ||
...
function collectDownloadResources(data: NonNullable<ProviderParsePayload['data']>): PublicImportResource[] {
  const collected: PublicImportResource[] = [];
  const seen = new Set<string>();

  const push = (resource: PublicImportResource | null) => {
    if (!resource) return;
    // pending: placeholders are unique per resource_content; real URLs dedupe later
    if (!resource.downloadUrl.startsWith('pending:') && seen.has(resource.downloadUrl)) return;
    if (!resource.downloadUrl.startsWith('pending:')) seen.add(resource.downloadUrl);
    ...
    mediaNodes.forEach((node, mediaIndex) => {
      ...
      if (nested.length > 0) {
        nested.forEach((raw, resIndex) => {
          const hint =
            total > 1
              ? `${mediaIndex + 1}/${total} ${String((asRecord(raw)?.type as string) || 'Media')}`
              : undefined;
...
function parseDurationSeconds(data: NonNullable<ProviderParsePayload['data']>): number | undefined {
...
async function callProvider(link: string, origin: 'source' | 'cache'): Promise<ProviderParsePayload> {
  ...
  const { data } = await axios.post<ProviderParsePayload>(env.IG_PARSE_URL, body.toString(), {
    timeout: 45_000,
    headers,
...
function humanizeProviderFailure(providerMessage: string): string {
  const msg = providerMessage.toLowerCase();
  if (msg.includes('analyze failed') || msg.includes('not found')) {
    return (
      `Instagram downloader could not analyze this reel (${providerMessage}). ` +
...
export class InstagramPublicUrlImportProvider implements PublicUrlImportProvider {
  ...
  async parse(url: string): Promise<PublicUrlParseResult> {
    ...
    try {
      ...
      if (cached) {
        return JSON.parse(cached) as PublicUrlParseResult;
      }
    } catch {
    ...
    const linkCandidates = Array.from(
      new Set([parsed.providerUrl, parsed.normalizedUrl].filter(Boolean)),
    );
    const attempts: Array<{ link: string; origin: 'source' | 'cache'; delayMs: number }> = [
      { link: linkCandidates[0]!, origin: 'source', delayMs: 0 },
      { link: linkCandidates[0]!, origin: 'source', delayMs: 1500 },
      ...(linkCandidates[1]
        ? [{ link: linkCandidates[1]!, origin: 'source' as const, delayMs: 500 }]
        : []),
    ...
    if (resources.length === 0) {
      throw new AppError(
        'No downloadable media found for this Instagram URL.',
        404,
        'INSTAGRAM_MEDIA_NOT_FOUND',
      );
    }
    ...
    const result: PublicUrlParseResult = {
   ...
   */
  async resolveResources(url: string, resourceIds: string[]): Promise<{
    ...
    const selected: PublicImportResource[] = [];
    for (const id of resourceIds) {
      const resource = byId.get(id);
      if (!resource) {
        throw new AppError(
          `Selected resource was not found. Preview may have expired — please preview again.`,
          400,
...
export function getPublicUrlImportProvider(url: string): PublicUrlImportProvider | null {
...
export function assertValidInstagramPublicUrl(url: string) {
  const parsed = parseInstagramMediaUrl(url);
  if (!parsed) {
    throw new AppError(
      'Invalid Instagram URL. Use a reel, post, or IGTV link.',
      400,
