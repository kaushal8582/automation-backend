...
function extractHttpUrl(payload: unknown): string | undefined {
 ...
 */
export async function resolveProviderDownloadUrl(
  raw: RawProviderResource,
): Promise<string | undefined> {
  const direct =
  ...
  const resourceContent =
    typeof raw.resource_content === 'string' ? raw.resource_content : undefined;
...
export async function ensureResourceDownloadUrl(
  resource: PublicImportResource,
  rawHint?: RawProviderResource,
): Promise<PublicImportResource> {
  if (resource.downloadUrl && /^https?:\/\//i.test(resource.downloadUrl)) {
    return resource;
  }
  if (!rawHint) return resource;
