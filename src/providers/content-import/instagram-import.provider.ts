...
function mapIgMediaType(node: IgMediaNode): string {
...
function toExternalItem(node: IgMediaNode, accountId: string): ExternalMediaItem {
  return {
    externalId: node.id,
    platform: 'instagram',
    accountId,
 ...
 */
export class InstagramImportProvider implements ContentImportProvider {
  ...
  async listMedia(input: ListMediaInput): Promise<ListMediaResult> {
    const limit = Math.min(
      Math.max(input.limit ?? CONTENT_IMPORT_DEFAULT_PAGE_SIZE, 1),
      IG_LIST_MAX_LIMIT,
    );

    try {
      const { data } = await this.client.get<IgMediaListResponse>(
  ...
  async getMediaDetails(input: GetMediaDetailsInput): Promise<ExternalMediaItem> {
    try {
      const { data } = await this.client.get<IgMediaNode>(`/${input.externalId}`, {
        params: {
          fields:
            'id,caption,media_type,media_product_type,media_url,thumbnail_url,permalink,timestamp',
          access_token: input.accessToken,
  ...
  async resolveMediaSource(input: ResolveMediaSourceInput): Promise<ResolvedMediaSource> {
    const item = input.item ?? (await this.getMediaDetails(input));
    if (!item.sourceUrl) {
      throw new AppError(
        'Instagram did not return a downloadable media URL for this item. It may be expired or unsupported.',
        404,
